...
public class VoiceController {
	...
	public String sendVoice(String msg,String lang,HttpServletRequest request)
	{
		...
		//mp3파일을 업로드할 위치 지정
		String path=request.getSession().getServletContext().getRealPath("/");//webapp에 저장
		String clientId = "a8n7dh0ufb";//애플리케이션 클라이언트 아이디값";
        String clientSecret = "zFbhlNkbrdJ8DT6OGdxknXYMMYUgoDZK7tbESCIF";//애플리케이션 클라이언트 시크릿값";
        try {
            ...
            if(responseCode==200) { // 정상 호출
                ...
                File f = new File(path+"/"+tempname + ".mp3");//webapp 에 mp3 파일 저장
                f.createNewFile();
